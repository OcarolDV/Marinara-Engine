...
- Dice notation is now read by one shared grammar in the four places that read it as a command — the `roll_dice` tool, the server and client sides of `/roll`, and the GM skill-check tag — instead of four private copies, so `roll_dice` accepts a bare `d20` like the other three already did. Notation whose roll could not be totalled exactly is now rejected as invalid instead of reporting a total that cannot be trusted — both a modifier too large to be a whole number and a modifier that is a whole number but large enough for the dice themselves to push the total past what can be counted exactly. A `/roll` too large for the engine's limits is still trimmed and rolled rather than refused, but its card, its narrator line and its stored result now name the dice actually thrown, so `/roll 500d6` reads `100d6` instead of claiming five hundred dice over a hundred. The `{{roll:XdY}}` macro and the dice-roll animation keep their own parsing and are unchanged.
