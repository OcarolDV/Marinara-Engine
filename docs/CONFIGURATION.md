...
| `MARINARA_BROWSER`                          | empty                                          | Browser the shell launchers open instead of the system default: a full executable path or a command on `PATH`, for example `C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe`, `/Applications/Brave Browser.app/Contents/MacOS/Brave Browser`, or `brave-browser`. The launcher passes the app URL as the only argument. If the command cannot be started, the launcher prints a warning and opens the default browser instead. Leave empty to use the default browser.                                                                          |
